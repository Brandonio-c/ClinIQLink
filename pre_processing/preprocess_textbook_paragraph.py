"""
PreprocessTextbook Module
Author: Brandon Colelough
Date: November 14, 2024
License: MIT

Description:
This module contains the `PreprocessTextbook` class, which processes a collection of textbook PDFs to create structured JSON files, each containing paragraph-level text extracted from each page. The `PreprocessTextbook` class reads a CSV catalog of textbook metadata, processes each specified PDF, extracts text into paragraphs, and saves the output as JSON files. Additionally, it generates a catalog CSV file that lists all processed JSON files, providing a summary of the processed textbooks for easy reference.

Usage:
    This script can be run from the command line with the following arguments:
    
        python preprocess_textbook.py <data_dir> <output_dir>
...
Example:
    To run the preprocessing on a data directory and save output to a specific directory:
        ...
        python preprocess_textbook.py /path/to/data_dir /path/to/output_dir
"""
import os
import csv
import re
...
class PreprocessTextbook:
    """
    A class to preprocess textbook PDFs into structured JSON files, splitting text by paragraphs and storing metadata.
    ...
    def process_page(self, page, page_num, json_data, title, isbn_13):
        try:
            ...
            for paragraph_num, paragraph in enumerate(paragraphs):
                ...
                paragraph_id = f"{isbn_13}-p{page_num+1}-para{paragraph_num+1}"
                section_id = f"{isbn_13}-p{page_num+1}"
                
                # Append paragraph data to JSON structure
